...
* **使用情境**：臺北市中山區圓山里（靠近松山機場航道），11 月晚間會有露天音樂會；希望在演出與收音時段**提前 10–30 秒**知道是否有飛機將「壓到頭頂」，並顯示**剩餘通過秒數**與**噪音等級（粗估）**，方便臨時降噪、靜音或橋段調度。
* **MVP 目標**（免費、非商用初版）：

  1. 即時輪詢附近飛機位置，
  2. 計算飛機與指定地點的**最近接近時間（ETA）**與**停留時長**，
  3. UI 顯示**倒數**與**預估持續秒數**，並列表未來幾架可能壓頂的航機。
* **成功指標**：
...
* 固定地點（可手動輸入/定位）之**飛機通過倒數**與**剩餘通過秒數**顯示
* 清單：未來 3–5 架可能進入半徑區域的航機（ETA / 持續秒數 / 呼號）
...
2. 每 **5–10 秒**輪詢 OpenSky 狀態向量（限 bbox）
3. 對每架機體做 CPA & 進出半徑 R 的時間解，產生：ETA、剩餘通過時長（若尚未進入半徑則等於預估通過秒數）、最近距離
...
* **ETA** = $\max(t_1, 0)$
* **剩餘通過秒數** = $\max(0, t_2 - \max(t_1,0))$
  * 若航機尚未進入半徑（$t_1 \ge 0$），此值亦代表完整的預估通過秒數。
...
* **高**：$d_g < 1200$ m、**中**：1200–2500 m、**低**：> 2500 m
...
* 次行：**剩餘通過 \~ 42 秒｜噪音：中｜信心：0.83**
* 地圖：地點圓圈（R）、航機即時點、預測線、CPA 點
* 清單（未來 3–5 架）：ETA / 剩餘通過秒數 / 呼號 / 估噪音
* **演出模式**：
