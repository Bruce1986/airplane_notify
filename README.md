...
* 地點 $P$：(lat, lon)；轉 ENU/UTM 等**局部平面座標**（MVP 規劃使用 `proj4js` 建立 WGS84→本地東北天座標的轉換函式，或以 Cesium/`@math.gl/geospatial` 提供的 ENU 工具）。**座標軸定義**：採東北天（ENU）局部平面，x 軸向東、y 軸向北、z 軸向上。
* 半徑 $R$：建議 **700 m**（後續現勘微調 500–1000 m）
* 飛機狀態：位置 $A_0$、地速 $v$（m/s）、**真航向** $θ$（弧度，採**自正北順時針**的測量方式以對應 ENU 定義）、高度 $h$（由 OpenSky `geo_altitude` 欄位（正規化後為 `geoAltitude`）優先取得；若為 `null` 則回退 `baro_altitude`（`baroAltitude`）；兩者皆缺則令 $h$ 為未定值）
...
* 估地面觀測距離 $d_g = \sqrt{d_{\min}^2 + h^2}$（假設 2D CPA 與當前高度同時發生）
* **高**：$d_g < 1200$ m、**中**：$1200 \le d_g < 2500$ m、**低**：$d_g \ge 2500$ m（數值可透過第 12 章「可設定」參數調整，以因應不同場地或飛機型號）
* 備註：若航機具顯著垂直速度，可利用 vertical_rate 將軌跡視為 3D 直線，對距離平方 $d(t)^2$ 求最小值以獲得更精準的 3D CPA 距離，再以此距離判斷噪音等級。
...
* **可設定**：地點座標、半徑 R、提醒門檻、輪詢頻率、夜間模式、噪音等級門檻（高/中與中/低分界值）
...
* `velocity`（m/s）→ 地速
* `true_track`（度，正規化為 `trueTrack`）→ 真航向（使用前請換算為弧度：`trackRad = trueTrack * Math.PI / 180`）
* `baro_altitude`/`geo_altitude`（m）→ 高度（**優先**使用 `geo_altitude`（`geoAltitude`）；若為 `null` 則回退至 `baro_altitude`（`baroAltitude`）；兩者皆缺時，視為高度未知並沿用第 5 章的保守噪音策略）
