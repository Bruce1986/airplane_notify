...
* **使用情境**：臺北市中山區圓山里（靠近松山機場航道），11 月晚間會有露天音樂會；希望在演出與收音時段**提前 10–30 秒**知道是否有飛機將「壓到頭頂」，並顯示**通過持續秒數**與**噪音等級（粗估）**，方便臨時降噪、靜音或橋段調度。
* **MVP 目標**（免費、非商用初版）：
...
* 固定地點（可手動輸入/定位）之**飛機通過倒數**與**通過持續秒數**顯示
...
* **OpenSky Network REST API**（匿名免費，有速率/範圍限制，研究/非商用友善）
  ...
  * 以\*\*地理方框（bbox）\*\*限制查詢範圍（例如：以圓山里為中心，半徑 \~10–15 km 等效 bbox）
...
3. 對每架機體做 CPA & 進出半徑 R 的時間解，產生：ETA、通過時長、最近距離
...
* 次行：**通過 \~ 42 秒｜噪音：中｜信心：0.83**
* 地圖：地點圓圈（R）、航機即時點、預測線、CPA 點
* 清單（未來 3–5 架）：ETA / 持續秒數 / 呼號 / 估噪音
* **演出模式**：
