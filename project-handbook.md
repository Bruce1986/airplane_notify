...
- **2025/11/25**
  - ✅ 完成：將 OpenSky 輪詢 hook 擴充為回傳錯誤狀態並調整 UI 顯示告警，確保資料中斷時能提醒使用者。
  - ✅ 完成：更新輪詢 hook 的單元測試，涵蓋錯誤情境與週期性請求行為，確保錯誤傳遞行為受到保護。

- **2025/11/24**
  - ✅ 完成：新增演出模式告警模組與單元測試，支援 T-30/T-10/通過中階段資訊提示。
  - ✅ 完成：更新 Demo 面板 UI，加入告警橫幅與動態標示，讓第一架航機狀態更一目了然。
  - ✅ 完成：統一秒數格式化工具，收斂重複實作並補上共用測試。
  - ✅ 完成：調整告警提示與航機清單樣式，統一 T-門檻文案與主動航機邊框呈現。
  - ✅ 完成：修正告警門檻正規化順序、改寫階段描述 switch 與精簡 AlertBanner props，回應 Gemini 建議。
  - ✅ 完成：依 Gemini 回饋補上 monitor case 與窮盡檢查，並調整 pass 項目邊框呈現，確保視覺一致性。
  - ✅ 完成：補齊淺色主題告警階段的陰影效果，確保與深色主題一致並回應 Gemini 建議。
  - ✅ 完成：修正進行中航機剩餘時間與告警 idle 判斷，確保 UI 與邏輯符合 Gemini 指示。
  - ✅ 完成：同步告警橫幅通過中剩餘時間計算與 PassItem 顯示邏輯一致，並補上相關單元測試。
  - ✅ 完成：以 max/min 簡化告警門檻正規化邏輯，提升程式碼可讀性並回應 Gemini 建議。
- **2025/11/23**
